...
- Inject one `<base>` pointing to `https://cdn.jsdelivr.net/gh/MuLin1/worldview-coordination@main/dist/V20260728/` before the fetched page is written.
...
- Static regression tests assert the loader contains the CDN base, no unsafe `window.location.href` URL base remains, no direct `vielsaenSpecies` reference remains, and local assets exist at their resolved paths.
